...
int main(){
	...
	for(int i_skip = 0; i_skip < coords_orig.size(); i_skip++){
		...
		bool brick_removable = true;

		for(int check_idx = 0; check_idx < i_skip; check_idx++){
			if(coords[check_idx] != coords_after_fall_orig[check_idx]) {
				brick_removable = false;
			}
		}
		for(int check_idx = i_skip+1; check_idx < coords.size()+1; check_idx++){
			if(coords[check_idx-1] != coords_after_fall_orig[check_idx]) brick_removable = false;
		}

		if(brick_removable) {
			ans++;
			cout<<i_skip<<" skipped, removable"<<endl;
		}
		else{
			cout<<i_skip<<" skipped, not removable"<<endl;
		}
	}
	cout<<ans<<endl;
