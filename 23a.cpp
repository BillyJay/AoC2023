...
void dfs(int i, int j){
	...
	if(j+1<141 && (field[i][j]=='>'||field[i][j]=='.')&&field[i][j+1]!='#'&&!visited[i][j+1]){
		int dist_temp =dist[i][j+1];
		dist[i][j+1] = max(dist[i][j+1], dist[i][j]+1);
		dfs(i,j+1);
		dist[i][j+1] = dist_temp;
	}
	if(j-1>=0 && (field[i][j]=='.')&&field[i][j-1]!='#'&&!visited[i][j-1]){
		int dist_temp = dist[i][j-1];
		dist[i][j-1] = max(dist[i][j-1], dist[i][j]+1);
		dfs(i,j-1);
		dist[i][j-1] = dist_temp;
	}
	if(i+1<141 && (field[i][j]=='v'||field[i][j]=='.')&&field[i+1][j]!='#'&&!visited[i+1][j]){
		int dist_temp = dist[i+1][j];
		dist[i+1][j] = max(dist[i][j+1], dist[i][j]+1);
		dfs(i+1,j);
		dist[i+1][j] = dist_temp;
	}
	if(i-1>=0 && (field[i][j]=='.')&&field[i-1][j]!='#'&&!visited[i-1][j]){
		int dist_temp = dist[i-1][j];
		dist[i-1][j] = max(dist[i][j+1], dist[i][j]+1);
		dfs(i-1,j);
		dist[i-1][j] = dist_temp;
