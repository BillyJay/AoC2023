...
int main(){
	...
	for(int i = 0; i < 1000; i++){
		...
		string patt_would_have = "";
		for(auto el : patt) patt_would_have = patt_would_have + to_string(el);
		patt_would_have = patt_would_have + to_string(BAppeared);

		if(patt.size()!=0) patt[patt.size()-1]+=BAppeared;
		else patt.push_back(BAppeared);
		string patt_str = "";
